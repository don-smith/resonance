...
Read `docs/architecture.md`, `src/packages/backlog/README.md`, `backlog/todo.yaml`, and the linked plan before changing a decision. Backlog is a repository-owned canonical data boundary with a constrained agent; do not turn it into general task management. The conversational agent may inspect the viewed repository through bounded read-only filesystem operations, but canonical Backlog access and all mutations stay behind domain tools.
...
The conversational agent must perform canonical Backlog reads and all mutations through its Backlog tools. It must re-read the selected canonical decision before each turn, read before writing, preserve YAML-to-plan linkage, and require explicit browser confirmation before deletion. It may use listing, bounded reads, glob, and grep across the viewed repository to gather evidence and understand relationships. Never expose credentials or inspect `.resonance/backlog-agent.env`; never grant generic filesystem writes, shell, network, or durable persistence access.
