...
export default function createArchitecture({ fetchFn = fetch, eventSourceFactory = (url) => typeof EventSource === 'function' ? new EventSource(url) : null } = {}) {
  ...
  let workspace; let agentPanel; let agentToggle; let transcript; let statusLabel; let promptInput; let sendButton; let contextUsage; let credentialPanel; let credentialInput; let retryButton;
  ...
  function updateViewHeader(viewId) { const view = views.find((candidate) => candidate.id === viewId); const title = root.querySelector('.architecture-view-title'); const level = root.querySelector('.architecture-view-level'); if (title) title.textContent = view?.name || viewId; if (level) level.textContent = view?.type || 'custom'; }
  ...
  function renderValidationResults(panel, results) { panel.textContent = ''; panel.append(element('h3', 'Results')); const list = element('ul'); for (const result of results) { const item = element('li', undefined, { class: `validation-${result.status}` }); item.append(element('strong', result.status.toUpperCase()), element('span', ` ${result.name}: ${result.message}`)); list.append(item); } panel.append(list); }
  function renderValidation() {
    ...
    const header = element('header'); const title = element('div'); title.append(element('p', 'ARCHITECTURE / CHECKS', { class: 'architecture-eyebrow' }), element('h2', 'Validation')); const button = element('button', 'Run validation', { type: 'button', class: 'architecture-validation-button' }); header.append(title, button); view.append(header);
    ...
    button.addEventListener('click', async () => { button.disabled = true; try { validationResults = (await json('/api/architecture/validation')).results; renderValidationResults(results, validationResults); } catch (error) { renderGraphError(error); } finally { button.disabled = false; } });
  }
