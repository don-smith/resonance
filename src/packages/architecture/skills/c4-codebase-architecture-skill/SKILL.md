...
### Keep architecture artifacts and Markdown current

When the user asks for an architecture or documentation change, use the available file editing tools to update the relevant architecture artifact or Markdown document. Prefer the repository's existing conventions and validate the result by reading it back. Do not edit implementation code; implementation files are outside this agent's write boundary.
