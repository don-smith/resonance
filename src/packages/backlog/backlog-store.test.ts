...
import { createBacklogStore, parseBacklogItems } from './backlog-store.ts';
...
test('parses strict source and rejects duplicate canonical plans', () => {
...
test('creates, edits, updates, and deletes a paired decision', async () => {
  ...
  try {
    assert.deepEqual(await store.createDecision({ title: 'New', plan: 'plans/new.md', status: 'in-planning', priority: 'P3', markdown: '# New' }), { affectedPaths: ['backlog/todo.yaml', 'backlog/plans/new.md'] });
...
test('rejects prospective symlink parents and serializes concurrent mutations', async () => {
  const root = await fixture(); const outside = await mkdtemp(path.join(tmpdir(), 'resonance-backlog-store-outside-'));
  try {
    await symlink(outside, path.join(root, 'backlog', 'escape'));
    const store = createBacklogStore({ repositoryRoot: root });
    await assert.rejects(() => store.createDecision({ title: 'Escape', plan: 'escape/new.md', status: 'in-planning', priority: 'P3', markdown: '# Escape' }), /escapes/);
...
test('compensates failed create and delete paired replacements', async () => {
  ...
  try {
    ...
    await assert.rejects(() => createStore.createDecision({ title: 'Rollback', plan: 'plans/rollback.md', status: 'in-planning', priority: 'P3', markdown: '# Rollback' }), /compensated/);
