...
const chatModulePath = new URL('../src/packages/chat/chat.js', import.meta.url);
...
async function loadCoordinator(window, document) {
  ...
  const modules = { '/assets/home/home.js': pathToFileURL(fileURLToPath(homeModulePath)).href, '/assets/docs/docs.js': pathToFileURL(fileURLToPath(docsModulePath)).href, '/assets/chat/chat.js': pathToFileURL(fileURLToPath(chatModulePath)).href };
...
test('loads Chat, preserves transcript across navigation, resets, and recovers its stream', async () => {
  const { window, document } = parseHTML('<!doctype html><head></head><body><nav id="primary-navigation"></nav><main id="package-mount"></main></body>');
  ...
  const fetchFn = async (url, options) => {
    ...
    if (url === '/api/manifest') return response({ version: 1, navigation: [{ id: 'home', label: 'Home', order: 10 }, { id: 'docs', label: 'Docs', order: 20 }, { id: 'chat', label: 'Chat', order: 30 }], packages: [
      ...
      { id: 'chat', entry: '/assets/chat/chat.js', stylesheet: '/assets/chat/chat.css' },
    ] });
    ...
    if (url === '/api/chat/prompt') return response({ accepted: true }, 202);
    if (url === '/api/chat/reset') return response({ ok: true, state: { messages: [{ id: 'reset-tool', role: 'activity', content: 'Reset tool · completed' }], status: 'idle', hasSession: false, error: null } });
  ...
  try {
    ...
    await application.activate('chat');
    assert.equal(FakeEventSource.instances.length, 1);
    assert.equal(FakeEventSource.instances[0].url, '/api/chat/events');
    ...
    assert.match(document.querySelector('.package-chat .chat-history').textContent, /hello/);
    await application.activate('docs');
    assert.equal(FakeEventSource.instances[0].closed, true);
    await application.activate('chat');
    assert.equal(FakeEventSource.instances.length, 2);
    assert.match(document.querySelector('.package-chat .chat-history').textContent, /hello/);
    const composer = document.querySelector('.package-chat .chat-composer');
    const input = composer.querySelector('textarea');
    input.value = 'next prompt';
    composer.dispatchEvent(new window.Event('submit', { bubbles: true, cancelable: true }));
    await new Promise((resolve) => setTimeout(resolve, 0));
    assert.ok(calls.some(([url, options]) => url === '/api/chat/prompt' && JSON.parse(options.body).prompt === 'next prompt'));
    FakeEventSource.instances[1].emit({ type: 'status', status: 'working' });
    assert.equal(document.querySelector('.package-chat .chat-wait').hidden, false);
    ...
    assert.equal(document.querySelector('.package-chat .chat-history').textContent.includes('Edit file'), false);
    assert.equal(document.querySelector('.package-chat .chat-history').textContent.includes('Delete file'), false);
    FakeEventSource.instances[1].emit({ type: 'message', message: { id: 'a1', role: 'assistant', content: 'answer' } });
    assert.match(document.querySelector('.package-chat .chat-history').textContent, /answer/);
    FakeEventSource.instances[1].emit({ type: 'done' });
    assert.equal(document.querySelector('.package-chat .chat-wait').hidden, true);
    document.querySelector('.package-chat .chat-new').click();
    await new Promise((resolve) => setTimeout(resolve, 0));
    assert.equal(document.querySelector('.package-chat .chat-history').textContent.includes('answer'), false);
    assert.equal(document.querySelector('.package-chat .chat-history').textContent.includes('Reset tool'), false);
    FakeEventSource.instances[1].fail();
    assert.equal(document.querySelector('.package-chat .chat-retry').hidden, false);
    document.querySelector('.package-chat .chat-retry').click();
    assert.equal(document.querySelector('.package-chat').dataset.status, 'error');
